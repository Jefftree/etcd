...
func TestNew(t *testing.T) {
	...
	var wb bytes.Buffer
	e := newEncoder(&wb, 0, 0)
	err = e.encode(&walpb.Record{Type: toPtr(CrcType), Crc: toPtr(uint32(0))})
	require.NoErrorf(t, err, "err = %v, want nil", err)
	err = e.encode(&walpb.Record{Type: toPtr(MetadataType), Data: []byte("somedata")})
	require.NoErrorf(t, err, "err = %v, want nil", err)
	r := &walpb.Record{
		Type: toPtr(SnapshotType),
		Data: pbutil.MustMarshal(&walpb.Snapshot{Index: toPtr(uint64(0)), Term: toPtr(uint64(0))}),
	}
	err = e.encode(r)
	require.NoErrorf(t, err, "err = %v, want nil", err)
	e.flush()
...
func TestOpenAtIndex(t *testing.T) {
	...
	w, err = Open(zaptest.NewLogger(t), dir, walpb.Snapshot{Index: toPtr(uint64(5))})
	require.NoErrorf(t, err, "err = %v, want nil", err)
...
func TestCut(t *testing.T) {
	...
	snap := walpb.Snapshot{Index: toPtr(uint64(2)), Term: toPtr(uint64(1)), ConfState: &confState}
...
func TestRecoverAfterCut(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		if err = md.SaveSnapshot(walpb.Snapshot{Index: toPtr(uint64(i)), Term: toPtr(uint64(1)), ConfState: &confState}); err != nil {
			t.Fatal(err)
		}
	...
	for i := 0; i < 10; i++ {
		w, err := Open(zaptest.NewLogger(t), p, walpb.Snapshot{Index: toPtr(uint64(i)), Term: toPtr(uint64(1))})
		if err != nil {
			if i <= 4 {
...
func TestValidSnapshotEntries(t *testing.T) {
	p := t.TempDir()
	snap0 := walpb.Snapshot{Index: toPtr(uint64(0)), Term: toPtr(uint64(0))}
	snap1 := walpb.Snapshot{Index: toPtr(uint64(1)), Term: toPtr(uint64(1)), ConfState: &confState}
	state1 := raftpb.HardState{Commit: 1, Term: 1}
	snap2 := walpb.Snapshot{Index: toPtr(uint64(2)), Term: toPtr(uint64(1)), ConfState: &confState}
	snap3 := walpb.Snapshot{Index: toPtr(uint64(3)), Term: toPtr(uint64(2)), ConfState: &confState}
	state2 := raftpb.HardState{Commit: 3, Term: 2}
	snap4 := walpb.Snapshot{Index: toPtr(uint64(4)), Term: toPtr(uint64(2)), ConfState: &confState} // will be orphaned since the last committed entry will be snap3
	func() {
		w, err := Create(zaptest.NewLogger(t), p, nil)
		if err != nil {
			t.Fatal(err)
